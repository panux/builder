...
type Sig struct {
	// Dat is the request data.
	Dat []byte `json:"dat"`

	// Key is the public key used to sign the request.
	// Key must be in PKCS1 format.
	Key []byte `json:"pubkey"`

	// Signature is the actual signature of the request.
	// Signature must be in PKCS1 format.
